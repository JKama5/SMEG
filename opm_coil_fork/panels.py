...
class PCB:  # call the other one BiplanarMesh
    ...
    def magnetic_field(self, target_points):
        ...
        # for layer in layers:
        #     line_conductor = LineConductor(self.loops[layer])
        #     field[layer] = line_conductor.magnetic_field(target_points)
        # return field['F.Cu'] - field['B.Cu']
...
class PCBPanel:
    ...
    def __init__(self, pcb_folder=None, half_names=None, standoff=None, rearrange=False, panel_dict=None):
        """A Panel is a collection of PCBs used to cancel one
        gradient/DC component.
    ...
    def plot_profile(self, current, profile_dir='x', field_component='x',
        ...
        plot_panel_profile(self, current, profile_dir, field_component,
                           spacing, min_pos, max_pos, ax, gradient)
...
def plot_panel_profile(panel, current, profile_dir='x', field_component='x',
    ...
    gradient : bool
        whether the panel is a gradient panel
    """
    if not ax:
        _, ax = plt.subplots()
    ...
    # if gradient:
    #     ax.set_ylabel(f'G{field_component} (nT)')
    # else:
    #     ax.set_ylabel(f'B{field_component} (nT)')
...
def load_panel(path, standoff=1.4, flip=[], rearrange=False):
    ...
    flip : list of strings
        names of which halves of the pcbs to flip.
    rearrange : bool
    ...
    for item in flip:
        if item in panel.chains.keys():
            flip_chains(panel.chains[item])
        else:
            print(f'Cannot flip {item}, not a valid panel.')
    return panel


def plot_panel(panel, target_size, n_points, current,
               axis, gradient=False, title=''):
    """Creates 2d colormap, line porfile, and arrow field map for coil.
    ...
    panel.plot_profile(current, profile_dir='z', gradient=gradient,
...
def plot_combined_panels(panels, currents, target_points):
